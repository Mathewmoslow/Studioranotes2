...
export default function InstructorTextbookPage() {
  ...
  // Textbook state - will be persisted to localStorage (instructor-specific key)
  ...
  useEffect(() => {
    setMounted(true)
    const saved = localStorage.getItem('studiora_instructor_textbook')
    if (saved) {
      try {
  ...
  const saveTextbook = (updated: TextbookState) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('studiora_instructor_textbook', JSON.stringify(updated))
    }
  }
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 4 }}>
        Organize your lecture notes and materials into chapters and export as a formatted PDF textbook.
      </Typography>
              ...
              {textbook.chapters.length === 0 ? (
                <Paper sx={{ p: 4, textAlign: 'center', backgroundColor: 'action.hover' }}>
                  ...
                  <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                    {notes.length > 0
                      ? 'Start by adding chapters, then assign your lecture notes to each chapter.'
                      : 'Create lecture notes first, then organize them into chapters here.'}
                  </Typography>
                  {notes.length > 0 ? (
                    <Button
                      variant="contained"
