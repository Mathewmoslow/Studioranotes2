...
// Query params: ?status=ACTIVE|COMPLETED|ARCHIVED|UPCOMING|ALL (default: ACTIVE)
export async function GET(request: NextRequest) {
  try {
    ...
    // Get status filter from query params (default to ACTIVE only)
    const { searchParams } = new URL(request.url)
    const statusFilter = searchParams.get('status')?.toUpperCase() || 'ACTIVE'
